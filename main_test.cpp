...
int main(int argc, char const *argv[])
{
    ...
    std::ofstream result_file("./results/test_tables_G.tex");
    std::ofstream result_file_V("./results/test_tables_V.tex");
    
    std::cout << "=== ТЕСТ: Генерация таблиц для задачи 3.11 ===\n" << std::endl;
    
    // Ограниченные параметры для теста
    std::vector<double> possible_mu = {0.1};
    std::vector<double> possible_C_rho = {1, 10};
    std::vector<int> modes = {1}; // только C*rho
    std::vector<double> tau_values = {0.1, 0.01};
    ...
    std::cout << "  - ./results/test_tables_G.tex (для G)" << std::endl;
    std::cout << "  - ./results/test_tables_V.tex (для V)" << std::endl;
