...
int main(int argc, char const *argv[])
{
    ...
    std::ofstream result_file("./results/tables_G.tex");
    std::ofstream result_file_V("./results/tables_V.tex");
    
    std::cout << "=== Генерация таблиц для задачи 3.11 ===\n" << std::endl;
                ...
                for (double tau : tau_values) {
                    ...
                    for (double h_x : h_values) {
                        ...
                        try {
                            ...
                            // Записываем результаты для G
                            result_file << "& $";
                            result_file.precision(6);
                            result_file << std::scientific << C_res_G << "$ \\\\\n";
                            result_file << "& $" << l2_res_G << "$ \\\\\n";
                            result_file << "& $" << w2_res_G << "$ \\\\\n";
                            result_file << "& $" << tau * h_x << "$ \\\\\n";
                            
                            // Записываем результаты для V
                            result_file_V << "& $";
                            result_file_V.precision(6);
                            result_file_V << std::scientific << C_res_V << "$ \\\\\n";
                            result_file_V << "& $" << l2_res_V << "$ \\\\\n";
                            result_file_V << "& $" << w2_res_V << "$ \\\\\n";
                            result_file_V << "& $" << tau * h_x << "$ \\\\\n";
    ...
    std::cout << "  - ./results/tables_G.tex (для G)" << std::endl;
    std::cout << "  - ./results/tables_V.tex (для V)" << std::endl;
